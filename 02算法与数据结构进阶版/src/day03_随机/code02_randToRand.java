...
/**
 * @author:fish
 * @date: 2023/4/25-22:15
 * @content: 01随机 和已存在随机机制下 再随机
 */
public class code02_randToRand {
    ...
    public static int rand01(RandomBox randomBox) {
        ...
        // min ~ max
        int size = max - min + 1;
        // size是不是奇数，odd 为奇数 返回true.
        boolean odd = (size & 1) != 0;
        int mid = size / 2;
        int ans = 0;
        do {
            ans = randomBox.random() - min;
        } while (odd && ans == mid); // 当前为奇数并且随机值为中位值，则重新随机
        return ans < mid ? 0 : 1;
    }
    ...
    public static void main(String[] args) {
        ...
        System.out.println("==========");
        System.out.println("等概率返回 0 1");
        int rand01;
        for (int i = 0; i < 25; i++) {
            rand01 = rand01(randomBox);
            System.out.print(rand01 + " ");
        }

        System.out.println("借助随机机制 等概率返回新的from 到to上的值 等概率");
        int ft;
        for (int i = 0; i < 25; i++) {
            ft = random(randomBox, 189, 300);
            System.out.print(ft + " ");
        }
